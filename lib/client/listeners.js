    ...
    'use strict';
     ...
     function ListenersProto(Util, DOM, CloudFunc, CloudCmd) {
        ...
        function onClick(event) {
            ...
            var panelClick = DOM.getPanelElement(event.target),
                panelActive = DOM.getByClass('panel active');
                
            if (panelClick != panelActive) { 
              if (event.target.dataset.name == 'js-files') {
                 DOM.changePanel(true);
              } else DOM.changePanel();
            }
        }
        ...
        function resize() {
            Events.add('resize', DOM.splitCheck);
            DOM.splitCheck();           
        }
