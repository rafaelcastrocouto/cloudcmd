        ...
        CmdProto                    = function() {
            ...
            this.isCurrentIsDir            = function(currentFile) {
                var current    = currentFile || this.getCurrentFile(),
                    ...
                    ret        = this.isContainClass(fileType, DIRECTORY_ICON);
           ...
           /**
            ...
            this.getPanelElement = function(element) {
                if (element)
            ...
            this.getPanel                = function(options) {
                ...
                if (!current) {
                  if (active) panel = this.getByDataName('js-left');
                  else panel = this.getByDataName('js-right');             
                } else {
                  panel = this.getPanelElement(current);
                  if (!active) {
                    var isLeft = panel.getAttribute('data-name') === 'js-left',
                    panel = this.getByDataName('js-' + (isLeft ? 'right' : 'left'));
                  }
                }
            ...
            this.changePanel            = function(noSelect) { 
                var dataName, files, current,
                    
                    panel           = DOM.getPanel({active: !noSelect}),
                    panelPassive    = DOM.getPanel({active: noSelect}),                    
                    name            = DOM.getCurrentName();
             
                if (noSelect) dataName  = panel.getAttribute('data-name');
                else dataName  = panelPassive.getAttribute('data-name');
                
                TabPanel[dataName]  = name;

                if (noSelect) {
                  dataName        = panelPassive.getAttribute('data-name');
                  name            = TabPanel[dataName];
                }

                if (name) { 
                    current     = DOM.getCurrentByName(name, panelPassive);
                    ...
                    if (!current && noSelect)
                        current = DOM.getFiles(panelPassive)[0];
                }
                ...
                panelPassive.classList.add('active');
            ...
            this.toggleSplit              = function() {
                DOM.getByClass('fm').classList.toggle('split');  
            }

            this.splitCheck               = function() {
                if (window.innerWidth <= 750) {
                  DOM.getByClass('fm').classList.remove('split');
                  DOM.getById('split').classList.add('hidden');
                }
                else {
                  DOM.getByClass('fm').classList.add('split');
                  DOM.getById('split').classList.remove('hidden')
                }
            }
